...
class ChatService {
	...
	constructor() {
		this.#ws.addEventListener(
			"error",
			() => alert("WebSocket hiba, zárd be a felesleges tabokat, és frissítsd az oldalt.")
		);
			...
			switch (p.type) {
				...
				case "message":
					...
					this.inbox!.conversations
						.find(x => x.channelId === cid)?.lastMessages
						.push(p.message);
					break;
	...
	addListener(listener: () => void) {
		...
		// nem akad össze a removeListener-rel, nem akarják ugyanazt változtatni
	}
